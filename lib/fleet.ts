...
// vehicle_tech_map sync lives in lib/fleet-vtm.ts (shared with
// /reports/vehicles' setVehicleDriver — every driver write routes through it).
import { syncVtm } from "@/lib/fleet-vtm";
...
export async function editVehicle(
  ...
  // Keep one tech <-> one vehicle: if a driver was set here, clear them off
  // others — and close those vtm intervals too (same landmine class as
  // setVehicleDriver: a vehicles_master-only write desyncs GPS attribution).
  const newDriver = update.primary_driver_short_name as string | null | undefined;
  if (typeof newDriver === "string" && newDriver) {
    const { data: others } = await supa.from("vehicles_master").select("id").eq("primary_driver_short_name", newDriver).neq("id", vehicleId);
    for (const o of (others ?? []) as Array<{ id: string }>) {
      await supa.from("vehicles_master").update({ primary_driver_short_name: null, updated_at: now }).eq("id", o.id);
      await syncVtm(supa, o.id, null, now);
    }
  ...
  if (newDriver !== undefined) {
    await syncVtm(supa, vehicleId, newDriver ?? null, now);
  }
  revalidatePath("/dispatch");
  return { ok: true };
}
