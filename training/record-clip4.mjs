...
const D = loadDurations("clip-4-estimate-piwm");
...
await r.beat("b1");
{
  ...
  await r.spotlight(optionCard, { hold: Math.round(total * 0.38), shot: "b1-option-card" });
  const lineRow = optionCard.locator("div.mb-2.rounded-md").first();
  await r.spotlight(lineRow, { hold: Math.round(total * 0.4), shot: "b1-cascade" });
}
  ...
  .locator('xpath=ancestor::div[contains(@class,"rounded-2xl")][1]');
{
  ...
  await sleep(Math.max(total - (Date.now() - t0), 500));
}
...
// ── b3: Start → the extract wait stays IN (narration teaches the ladder) ────
{
  ...
  await r.beat("b3"); // mp3 lands the moment the wait begins
...
await r.beat("b6");
{
  ...
  await r.spotlight(firstCard, { hold: Math.round(total * 0.28), shot: "b6-proposed" });
  ...
  await r.spotlight(injected, { hold: Math.round(total * 0.34), pulses: 4, shot: "b6-injected-line" });
  await r.parkCursor();
  await sleep(Math.max(Math.round(total * 0.12), 1200));
}
